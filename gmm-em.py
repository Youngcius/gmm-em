...
class GmmCluster(object):
    ...
    def cluster(self,X):
        ...
        for i in range(self.iteration): # e.g. 1000次迭代
            ...
            for k in range(self.K): # 对第 k 个高斯模型
                self.sigma[k] = np.dot((X-self.mu[k]).transpose()*self.gamma[:,k], X-self.mu[k]) / np.sum(self.gamma[:,k])
                # self.alpha[k] =
    ...
    def gaussian(self,x,k):
        x = x.ravel() # x 为一维向量
        return np.exp(-(np.dot(np.dot(x-self.mu[k], np.linalg.pinv(self.sigma[k])), x-self.mu[k]))/2) / np.power(2*np.pi, self.K/2) / np.sqrt(np.linalg.det(self.sigma[k]))
...
plt.subplot(2,2,1)
sns.scatterplot(data[:,0],data[:,1],style=target)
plt.title("True")
plt.subplot(2,2,2)
sns.scatterplot(data[:,2],data[:,3],style=target)
plt.title("True")
plt.subplot(2,2,3)
sns.scatterplot(data[:,0],data[:,1],style=km_pred)
plt.title("Cluster")
plt.subplot(2,2,4)
sns.scatterplot(data[:,2],data[:,3],style=km_pred)
plt.title("Cluster")
